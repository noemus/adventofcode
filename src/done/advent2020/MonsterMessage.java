    ...
    public static void main(String[] args) {
        try (Scanner in = new Scanner(INPUT2)) {
            rules = Stream.generate(new LineSupplier(in))
                          ...
                          .map(Rule::of)
                          .sorted(Comparator.comparing(Rule::index))
                          .toArray(Rule[]::new);

            messages = Stream.generate(new LineSupplier(in))
...
//                             .peek(System.out::println)
            ...
            Stream.of(rules)
                  .forEach(rule -> System.out.println(rule.index() + ": " + rule.toRegex()));

            System.out.println("Rules count: " + rules.length);
            System.out.println("Rules max index: " + rules[rules.length - 1].index());

            Pattern regex = Pattern.compile(rules[0].toRegex());

            final String regex42 = rules[42].toRegex();
            final String regex31 = rules[31].toRegex();
            Pattern regex2 = Pattern.compile("(" + regex42+ "){2,}(" + regex31 + ")+");

            long result = Stream.of(messages)
                                .map(regex::matcher)
                                .filter(Matcher::matches)
                                //.filter(matcher -> matcher.end("left") - matcher.start("left") == matcher.end("right") - matcher.start("right"))
                                .count();
    ...
    static class CharRule implements Rule {
        final int index;
        final char matchedChar;

        public CharRule(int ruleIdx, char c) {
            this.index = ruleIdx;
            this.matchedChar = c;
        }
        ...
        public String toRegex() {
            return String.valueOf(matchedChar);
        }
    }

    static class ComplexRule implements Rule {
        final int index;
        final List<int[]> subrules;

        ComplexRule(int ruleIdx, List<int[]> subrules) {
            this.index = ruleIdx;
        ...
        public String toRegex() {
            if (index == 8) {
                int[] branch1 = subrules.get(0);
                return "(" + Rule.rulesToRegex(branch1) + ")+";
            }
//            if (index == 11) {
//                int[] branch1 = subrules.get(0);
//                String left = rules[branch1[0]].toRegex();
//                String right = rules[branch1[1]].toRegex();
//                return "(?<left>(?:"+left+")+)(?<right>(?:"+right+")+)";
//            }
            return subrules.stream()
                           .map(Rule::rulesToRegex)
                           .collect(joining("|", "(", ")"));
        }
    }
    ...
    interface Rule {
        ...
        static String rulesToRegex(int[] ruleList) {
            return IntStream.of(ruleList)
                            .mapToObj(r -> rules[r])
                            .map(Rule::toRegex)
                            .collect(joining());
        }
        ...
        static Rule of(String line) {
            ...
            int ruleIdx = Integer.parseInt(line.substring(0, colon));
            String rule = line.substring(colon + 1).trim();
            if (rule.charAt(0) == '"') {
                return new CharRule(ruleIdx, rule.charAt(1));
            }
            List<int[]> subrules = Stream.of(rule.split("[|]"))
                                         .map(String::trim)
                                         .filter(not(String::isBlank))
                                         .map(Rule::parseBranch)
                                         .collect(toList());
            return new ComplexRule(ruleIdx, subrules);
        }

        private static int[] parseBranch(String branch) {
            return Stream.of(branch.split(" "))
                         .map(String::trim)
    ...
    @Test
    public void test() {
//        assertTrue("ababbb".matches("(a((aa)|(bb)(ab)|(ba))|((ab)|(ba)(aa)|(bb))b)"));
//        assertTrue("ababbb".matches("a(((ab|ba)(aa|bb))|((aa|bb)(ab|ba)))b"));

        Matcher matcher = Pattern.compile("(?:(?<left>(?:(?:a)+)))(?:(?<right>(?:(?:b)+)))").matcher("aaabbb");
        assertTrue(matcher.matches());
        assertEquals("aaa", matcher.group("left"));
        assertEquals(3, matcher.end("left") - matcher.start("left"));
        assertEquals("bbb", matcher.group("right"));
        assertEquals(3, matcher.end("right") - matcher.start("right"));

        Pattern.compile("""
(?:(?:(?:(?:(?:a(?:(?:(?:a(?:b(?:ba)|a(?:bb))|b(?:(?:bb)b|(?:(?:a|b)b|aa)a))a|(?:a(?:(?:bb|ab)(?:a|b))|b(?:b(?:bb|ba)|a(?:ab|aa)))b)b|(?:b(?:a(?:b(?:bb|aa)|a(?:(?:a|b)b|aa))|b(?:b(?:bb|ba)|a(?:ab|aa)))|a(?:(?:b(?:ba)|a(?:bb|ab))a|(?:(?:bb|ab)b|(?:ba|a(?:a|b))a)b))a)|b(?:b(?:b(?:a(?:b(?:ab|ba)|a(?:ab))|b(?:(?:ab|aa)b|(?:ab|ba)a))|a(?:a(?:(?:ab)b|(?:(?:a|b)(?:a|b))a)|b(?:(?:ba|a(?:a|b))a)))|a(?:b(?:b(?:(?:(?:a|b)a|bb)b|(?:ab|ba)a)|a(?:(?:bb|aa)a|(?:(?:a|b)(?:a|b))b))|a(?:b(?:(?:(?:a|b)a|bb)a|(?:bb|ab)b)|a(?:(?:ab|aa)(?:a|b))))))b|(?:(?:a(?:(?:b(?:a(?:(?:a|b)(?:a|b))|b(?:(?:a|b)b|aa))|a(?:b(?:(?:a|b)a|bb)))b|(?:(?:(?:ab)b|(?:(?:a|b)b|aa)a)a|(?:(?:(?:a|b)a|bb)a|(?:ab)b)b)a)|b(?:b(?:(?:a(?:(?:a|b)a|bb)|b(?:aa))b|(?:a(?:bb|ab)|b(?:(?:a|b)a|bb))a)|a(?:b(?:(?:(?:a|b)(?:a|b))b|(?:ab)a)|a(?:b(?:ab)|a(?:ba)))))b|(?:(?:(?:(?:b(?:bb|ba)|a(?:ab|aa))b|(?:(?:ab|ba)b)a)b|(?:a(?:(?:ab|aa)b|(?:bb|ba)a)|b(?:b(?:bb|ba)|a(?:(?:a|b)(?:a|b))))a)a|(?:b(?:a(?:a(?:ab)|b(?:aa))|b(?:(?:bb|ab)b|(?:ab|ba)a))|a(?:b(?:(?:ab|aa)a|(?:ab)b)|a(?:(?:ab|aa)a|(?:aa)b)))b)a)a))+)\
(?<left>(?:(?:(?:a(?:(?:(?:a(?:b(?:ba)|a(?:bb))|b(?:(?:bb)b|(?:(?:a|b)b|aa)a))a|(?:a(?:(?:bb|ab)(?:a|b))|b(?:b(?:bb|ba)|a(?:ab|aa)))b)b|(?:b(?:a(?:b(?:bb|aa)|a(?:(?:a|b)b|aa))|b(?:b(?:bb|ba)|a(?:ab|aa)))|a(?:(?:b(?:ba)|a(?:bb|ab))a|(?:(?:bb|ab)b|(?:ba|a(?:a|b))a)b))a)|b(?:b(?:b(?:a(?:b(?:ab|ba)|a(?:ab))|b(?:(?:ab|aa)b|(?:ab|ba)a))|a(?:a(?:(?:ab)b|(?:(?:a|b)(?:a|b))a)|b(?:(?:ba|a(?:a|b))a)))|a(?:b(?:b(?:(?:(?:a|b)a|bb)b|(?:ab|ba)a)|a(?:(?:bb|aa)a|(?:(?:a|b)(?:a|b))b))|a(?:b(?:(?:(?:a|b)a|bb)a|(?:bb|ab)b)|a(?:(?:ab|aa)(?:a|b))))))b|(?:(?:a(?:(?:b(?:a(?:(?:a|b)(?:a|b))|b(?:(?:a|b)b|aa))|a(?:b(?:(?:a|b)a|bb)))b|(?:(?:(?:ab)b|(?:(?:a|b)b|aa)a)a|(?:(?:(?:a|b)a|bb)a|(?:ab)b)b)a)|b(?:b(?:(?:a(?:(?:a|b)a|bb)|b(?:aa))b|(?:a(?:bb|ab)|b(?:(?:a|b)a|bb))a)|a(?:b(?:(?:(?:a|b)(?:a|b))b|(?:ab)a)|a(?:b(?:ab)|a(?:ba)))))b|(?:(?:(?:(?:b(?:bb|ba)|a(?:ab|aa))b|(?:(?:ab|ba)b)a)b|(?:a(?:(?:ab|aa)b|(?:bb|ba)a)|b(?:b(?:bb|ba)|a(?:(?:a|b)(?:a|b))))a)a|(?:b(?:a(?:a(?:ab)|b(?:aa))|b(?:(?:bb|ab)b|(?:ab|ba)a))|a(?:b(?:(?:ab|aa)a|(?:ab)b)|a(?:(?:ab|aa)a|(?:aa)b)))b)a)a))+)\
(?<right>(?:(?:(?:a(?:b(?:(?:b(?:(?:ba)b|(?:bb|ba)a)|a(?:(?:(?:a|b)a|bb)a))b|(?:(?:(?:bb|ab)a|(?:(?:a|b)b|aa)b)a|(?:b(?:(?:a|b)(?:a|b))|a(?:ab))b)a)|a(?:a(?:b(?:(?:bb)a|(?:(?:a|b)(?:a|b))b)|a(?:(?:a|b)(?:bb|aa)))|b(?:a(?:(?:(?:a|b)(?:a|b))a|(?:bb|ba)b)|b(?:a(?:ba)|b(?:ba)))))|b(?:(?:a(?:b(?:(?:(?:a|b)a|bb)a)|a(?:(?:(?:a|b)b|aa)b|(?:(?:a|b)a|bb)a))|b(?:a(?:b(?:aa)|a(?:bb|aa))|b(?:b(?:bb|ab)|a(?:bb|ba))))b|(?:a(?:b(?:b(?:(?:a|b)b|ba)|a(?:bb))|a(?:(?:(?:a|b)a|bb)b|(?:bb|ab)a))|b(?:(?:a(?:ba|a(?:a|b))|b(?:ab))a|(?:b(?:ab|ba)|a(?:ab|aa))b))a))a|(?:a(?:a(?:(?:a(?:b(?:aa)|a(?:(?:a|b)(?:a|b)))|b(?:b(?:ba)|a(?:(?:a|b)(?:a|b))))b|(?:b(?:b(?:ab|ba)|a(?:(?:a|b)a|bb))|a(?:b(?:aa)|a(?:bb|aa)))a)|b(?:a(?:(?:a(?:ba)|b(?:ab|ba))a|(?:(?:bb|ba)b|(?:bb|ab)a)b)|b(?:(?:(?:ab|aa)(?:a|b))b|(?:a(?:bb|ba)|b(?:ab|aa))a)))|b(?:(?:b(?:b(?:a(?:bb|ab)|b(?:bb|ba))|a(?:(?:(?:a|b)(?:a|b))a|(?:(?:a|b)b|ba)b))|a(?:(?:(?:ab)a|(?:ab|aa)b)a|(?:a(?:ab)|b(?:ba))b))b|(?:(?:b(?:(?:ba)b|(?:bb|ba)a)|a(?:b(?:ab|ba)|a(?:ab)))a|(?:b(?:(?:ba)a|(?:bb|ab)b)|a(?:(?:ab|ba)a|(?:bb|aa)b))b)a))b))+))
""");

//        assertTrue("aaabbb".matches("(a)(?:\1\2)*(b)"));
    }
}
// 31: 99 123 | 63 97
// 42: 119 97 | 58 123

// (119 97 | 58 123) (99 123 | 63 97)